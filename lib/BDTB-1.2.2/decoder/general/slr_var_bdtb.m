function [results, pars] = slr_var_bdtb(D, pars)
%slr_var_bdtb - performs SLR-VAR-1vsR using SLR1.2.1alpha, either train or test
%[results, pars] = slr_var_bdtb(D, pars)
%
% Input:
...
%% Train mode:
    ...
    if num_class==2
        if isempty(ax0)
            [weight, ix_eff] = slr_learning_var2(label2, data2, 'nlearn',nlearn, 'amax',amax);
        else
            [weight, ix_eff] = slr_learning_var2(label2, data2, 'nlearn',nlearn, 'ax0',ax0, 'amax',amax);
        end
        [pred, dec_val] = calc_label(data2, [zeros(size(data2,2),1) weight]);
    else
        ...
        for itc=1:num_class
            ...
            if isempty(ax0)
                [ww, ix] = slr_learning_var2(label_temp, data2, 'nlearn',nlearn, 'amax',amax);
            else
                [ww, ix] = slr_learning_var2(label_temp, data2, 'nlearn',nlearn, 'ax0',ax0, 'amax',amax);
            end
            weight(:,itc) = ww;
            ix_eff{itc}   = ix;
        end
