function [D, pars]  = removeBlockSample(D, pars)
% removeBlockSample - remove samples in each block for each channel (voxel)
...
%   pars.begin_off     - start ind offset of samples to be included from the beginning of each block
%   pars.end_off       - end ind offset of samples to be included from the end of each block
% Optional:
    ...
    if ismember(tmp,target_labels)
        ...
        if bi>ei
            if exist('inds_runs','var') && ismember(inds_blocks(2,itb),inds_runs(2,:))
                ...
                fprintf('\nWarning: End-point of block removing is smaller than begin-point\n Use only begin-point\n');
                ei = bi;
            else
                error('begin/end_off is too many to keep samples of block removing');
            end
        end
        
        data_temp{itb} = D.data(bi:ei,:);
    else
