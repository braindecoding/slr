...
for j = 1:length(cvIdx.training.runIdx)
  ...
  if strfind(decoderName, '1x2')
    fname{k} = [basePath '1x2_' trainStr]; labelList{k}=[0 1 2]/basisNormCoef(2); k=k+1;
  end
  if strfind(decoderName, '2x1')
    fname{k} = [basePath '2x1_' trainStr]; labelList{k}=[0 1 2]/basisNormCoef(3); k=k+1;
  end
  if strfind(decoderName, '2x2')
    fname{k} = [basePath '2x2_' trainStr]; labelList{k}=[0 1 2 3 4]/basisNormCoef(4); k=k+1;
  end
