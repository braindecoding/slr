function calCombCoef_randRecon_smlr(decoCombType,optMode,imgCvRunSetIdx,roiArea)
...
cvModeLocalDecoder = 'leave2';
cvModeImgRecon = 'trainCombRandRecon';
...
saveFname = [sbjID '_' roiName '_baseByRestPre_' decType '_' ...
    ...
    saveFnamePostfix2 '_imgTestIdx' num2str(imgCvRunSetIdx) '_randReconW.mat'];

saveFnameToSkip = [sbjID '_' roiName '_baseByRestPre_' decType '_' ...
    saveFnamePostFix '_linComb-' optMode '_' decoCombType ...
    saveFnamePostfix2 '_imgTestIdx' num2str(imgCvRunSetIdx) '_randReconW_toSkip.mat'];
...
[decoderAllCv, basisMat, decoCombType, cvIdxSet] ...
    = prepareDecoder_basisNormalized(randImgRun, decoderPath,decoCombType,basisNormMode,cvModeImgRecon,imgCvRunSetIdx);

%% extract data of figure image data
[smplFigTest, stimFigTest] = getNoRestData(D_1x1,figTestRun);
labelFigTest = stimFigTest(:,1); 
stimFigTest = stimFigTest(:,2:end)/onVal; 

%% extract data of rand image data
...
for smplIdx = 1:size(xImgTestAll,3)
    stimImgTestAllPre(smplIdx,:) = xImgTestAll(:,:,smplIdx) * w;
end
